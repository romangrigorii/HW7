...
void LCD_drawColumn(unsigned char val, unsigned short x, unsigned short y, unsigned short coloron,
        unsigned short coloroff){
    int i;
    for (i=0;i<8;i++){
        if ((val>>i)&0x01){
            LCD_drawPixel(x, y+i,coloron);
        }
        else{
            LCD_drawPixel(x, y+i,coloroff);
        }
    }
}
void LCD_writeChar(unsigned char letter,unsigned short x, unsigned short y,unsigned short color,
        unsigned short bckcolor){
    int u;
    for (u=0;u<5;u++){
        LCD_drawColumn(ASCII[letter-32][u],x+u,y,color,bckcolor);
    }
}
